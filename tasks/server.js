...
gulp.task('CLAY_BROWSER', ['CLAY_BUILD'], function() {
  buildFolder = config.get("build.folder");
  browserSync({
    server: {
      // src is included for use with sass source maps
      baseDir: [ config.get("build.folder") ]
...
gulp.task('CLAY_SERVER', ['CLAY_BROWSER'], function() {
  console.log( config.get("ASSETS_GLOBS") )
  gulp.watch( config.get("build.src") + '/*.css', ['CLAY_CSS']);
  gulp.watch( config.get("build.src") + '/css/**', ['CLAY_CSS']);
